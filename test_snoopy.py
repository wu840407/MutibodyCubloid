...
def duck(d):
    pcd = o3d.io.read_point_cloud("./dataset/SnoopyCanonical.ply")
    pcd = o3d.geometry.PointCloud.uniform_down_sample(pcd, 20)
    R = pcd.get_rotation_matrix_from_xyz(( 0, float(d),0))
...
if __name__ == '__main__':
    ...
    test_set = shapenet4096('test', "D:/Project/PythonProject/CuboidAbstractionViaSeg-main/E_shapenet4096/", 'table', True)
    in2=[]
    in2.append(duck("0").unsqueeze(0))
    in2.append(duck("1").unsqueeze(0))
    in2.append(duck("2").unsqueeze(0))
    in2.append(duck("5").unsqueeze(0))
    test_loader = DataLoader(in2, batch_size=1, shuffle=False, num_workers=1, pin_memory=True)
    ...
    with tqdm.tqdm(enumerate(test_loader, 0), total=len(test_loader), desc='test') as tbar:
        for i, data in tbar:
            with torch.no_grad():
                ...
                if i==3:                    
                    out=torch.cat((in1[0],in1[1], in1[2],in1[3]), dim=1)
                    
                    pd_segm,_,_= model(out)

                    segmented_pcds = []

                    for view_i in range(out.size(1)):
                            # 計算miou
                        segmented_pcds.append(build_pointcloud(out[0, view_i].cpu().numpy(),
                                                                       pd_segm.argmax(-1)[0, view_i].cpu().numpy()))
                        segmented_pcds[-1].translate([view_i*0.4, 0.0, 0.0])
