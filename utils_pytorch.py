...
def generate_cube_mesh_batch(vertices, cube_face):
...
def visualize_segmentation(pc, color, assign_matrix, save_path, bias):
    batch_size = pc.shape[0]

    pc = pc.cpu().detach().numpy()

    assign_matrix = assign_matrix.cpu().numpy()
    filename = save_path + str(bias)  + '_segment.ply'
    color_segment = color[np.argmax(assign_matrix,1),:]

    export_pc(pc, color_segment, filename)
