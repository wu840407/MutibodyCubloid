...
def parsing_hyperparas(args):
    ...
    hypara = {}
    hypara['W'] = {}
    hypara['W']['W_REC']=args.W_REC
    hypara['W']['W_std']=args.W_std
    hypara['W']['W_SPS']=args.W_SPS
    hypara['W']['W_EXT']=args.W_EXT
    hypara['W']['W_KLD']=args.W_KLD
    hypara['W']['W_CST']=args.W_CST
    return hypara
...
def get_cub_train_specs(hypara):
    from utils.pointnet2_util import furthest_point_sample
    criterion = CombinedLoss(names=['trans', 'group','cub'], loss=[MotTransLoss(), MotGroupLoss(),CubLoss(hypara)], weights=[1.0, 1.0, 1.0])
    def forward_fn(network, data, is_eval=False):
        with torch.set_grad_enabled(not is_eval):
            inputs, flow, segm = data
            inputs = inputs.cuda()
            ...
            sub_ind1 = furthest_point_sample(inputs[:, 1, ...].contiguous(), 256).long()
            sub_inds = torch.stack([sub_ind0, sub_ind1], dim=1)
            flow = torch.where(torch.isnan(flow),torch.full_like(flow,0),flow)
            flow = torch.where(torch.isinf(flow),torch.full_like(flow,1),flow)
            pd_group_matrix, res0, res1, outdict = network(inputs, flow, sub_inds)
...
if __name__ == '__main__':
    ...
    if args.type == "flow":
        ...
        from models.cub_net import CubNet
        model = CubNet().cuda()
        model_fn, model_data_spec, model_view_sel = get_cub_train_specs(hypara)
    
    elif args.type == "cub-original":
        hypara= parsing_hyperparas(args)
        from models.cub_net_original import CubNet_Original
        model = CubNet_Original().cuda()
        model_fn, model_data_spec, model_view_sel = get_cub_original_train_specs(hypara)
