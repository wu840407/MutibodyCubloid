...
class Attention_module(nn.Module):
    def __init__(self, attention_dim):
        ...
        self.conv_R = nn.Sequential(nn.Conv1d(16, self.attention_dim, kernel_size=1, bias=False))
        self.conv_T = nn.Sequential(nn.Conv1d(16, self.attention_dim, kernel_size=1, bias=False))
    ...
    def forward(self, x_per, x_cuboid, rotate, trans):
        ...
        R = self.conv_R(rotate.reshape(-1,16,9)).transpose(2, 1)
        T = self.conv_T(trans).transpose(2, 1)

        assign_matrix = F.softmax(torch.matmul(K,Q), dim = 1).transpose(2, 1)  # (batch_size, num_point, num_cuboid*6)
        assign_rotate = F.softmax(torch.matmul(R,Q), dim = 1).transpose(2, 1)
        assign_trans = F.softmax(torch.matmul(T,Q), dim = 1).transpose(2, 1)
...
class MiniPointNets(nn.Module):
    ...
    def forward(self, xyz, flow, sub_inds, pred_trans, assign_rotate, assign_trans):
        ...
        trans0_down = gather_nd(assign_trans[:, 0, ...], sub_ind0)
        rotate0_down = gather_nd(assign_rotate[:, 0, ...], sub_ind0)

        xyz1_down = gather_nd(xyz[:, 1, ...], sub_ind1)
        flow1_down = gather_nd(flow[:, 1, ...], sub_ind1)
        trans1_down = gather_nd(assign_trans[:, 1, ...], sub_ind1)
        rotate1_down = gather_nd(assign_rotate[:, 1, ...], sub_ind1)

        Rs0 = rotate0_down.reshape(-1, nsample, 3, 3)
        ts0 = trans0_down.reshape(-1, nsample, 3)
        ...
        Rs1 = rotate1_down.reshape(-1, nsample, 3, 3)
        ts1 = trans1_down.reshape(-1, nsample, 3)
...
class CubNet(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, flow: torch.Tensor, sub_inds: torch.Tensor):
        ...
        assign_rotate = assign_rotate.reshape(n_batch, 2, n_point, -1)
        assign_trans = assign_trans.reshape(n_batch, 2, n_point, -1)

        group_matrix, res0, res1 = self.verify_net(xyz, flow, sub_inds, pred_trans, assign_rotate, assign_trans)
