...
class FullNet(nn.Module):
    ...
    def load_subnet_weights(self):
        ...
        with Path(self.args.cub_model).open() as f:
            cub_config = yaml.load(f, Loader=yaml.FullLoader)
        self.cub_net.load_state_dict(torch.load(cub_config["save_path"] + "/best.pth.tar")['model_state'])
    ...
    def forward(self, xyz):
        ...
        for view_i in range(n_view):
            for view_j in range(view_i + 1, n_view):
                ...
                motion_ij, _, _, _=self.cub_net(xyz[:, [view_i, view_j]], flow_ij, sub_inds[:, [view_i, view_j]])
        ...
        motion_absolute=torch.clamp(motion_absolute,0,self.train_s)
