...
class Feature_extract(nn.Module):
    def __init__(self, emb_dims, z_dims, k, num_cuboid, low_dim_idx):
        ...
        self.bn1_2 = nn.BatchNorm2d(120)
        self.bn2_1 = nn.BatchNorm2d(64)
        self.bn2_2 = nn.BatchNorm2d(120)
        self.bn3_1 = nn.BatchNorm1d(self.emb_dims)
        self.bn4_1 = nn.BatchNorm1d(self.z_dims)
        self.conv1 = nn.Sequential(nn.Conv2d(6 , 64, kernel_size=1, bias=False),
                                   self.bn1_1,
                                   nn.LeakyReLU(negative_slope=0.2, inplace = True),
                                   nn.Conv2d(64, 120, kernel_size=1, bias=False),
                                   self.bn1_2,
                                   nn.LeakyReLU(negative_slope=0.2, inplace = True))
        self.conv2 = nn.Sequential(nn.Conv2d(120*2, 64, kernel_size=1, bias=False),
                                   self.bn2_1,
                                   nn.LeakyReLU(negative_slope=0.2, inplace = True),
                                   nn.Conv2d(64, 120, kernel_size=1, bias=False),
                                   self.bn2_2,
                                   nn.LeakyReLU(negative_slope=0.2, inplace = True))
        self.conv3 = nn.Sequential(nn.Conv1d(240, self.emb_dims, kernel_size=1, bias=False),
                                   self.bn3_1,
                                   nn.LeakyReLU(negative_slope=0.2, inplace = True))
        ...
        self.enc_cuboid_vec = nn.Sequential(nn.Conv1d(self.num_cuboid, 64, kernel_size=1, bias=False),
                                            nn.LeakyReLU(negative_slope=0.2, inplace = True))
        self.conv_cuboid = nn.Sequential(nn.Conv1d(self.z_dims + 64, 512, kernel_size=1, bias=False),
                                         nn.LeakyReLU(negative_slope=0.2, inplace = True),
                                         nn.Conv1d(512, 240, kernel_size=1, bias=False),
                                         nn.LeakyReLU(negative_slope=0.2, inplace = True))
...
class Para_pred(nn.Module):
    def __init__(self):
        ...
        self.conv_scale = nn.Conv1d(240, 3, kernel_size=1)
        nn.init.zeros_(self.conv_scale.bias)

        self.conv_rotate = nn.Conv1d(180, 4, kernel_size=1)
        self.conv_rotate.bias.data = torch.Tensor([1, 0, 0, 0])

        self.conv_trans = nn.Conv1d(60, 3, kernel_size=1)
        nn.init.zeros_(self.conv_trans.bias)

        self.conv_ext =   nn.Sequential(nn.Conv1d(240, 32, kernel_size=1, bias=True),
                                        nn.LeakyReLU(negative_slope=0.2, inplace = True),
    ...
    def forward(self, x_cuboid):
        ...
        rotate = self.conv_rotate(x_cuboid[:, :180,:]).transpose(2, 1)  # (batch_size, num_cuboid, 4)
        rotate = quat2mat(F.normalize(rotate,dim=2,p=2))     # (batch_size, num_cuboid, 3, 3)

        trans = self.conv_trans(x_cuboid[:, 180:,:]).transpose(2, 1)    # (batch_size, num_cuboid, 3)
...
class Attention_module(nn.Module):
    def __init__(self, attention_dim):
        ...
        self.conv_Q = nn.Sequential(nn.Conv1d(240, self.attention_dim, kernel_size=1, bias=False))
        self.conv_K = nn.Sequential(nn.Conv1d(240, self.attention_dim, kernel_size=1, bias=False))
